  ...
  if (carts) {
    if (count == 0) {
      ...
      await cartModel.updateOne(
        { customerID: user.id },
        { $push: { cart: { productID: productID, qty: count } } }
      )
    } else {
      await cartModel.updateOne(
        { customerID: user.id, 'cart.productID': productID },
        { $set: { 'cart.$.qty': count } }
      )
    }
  } else {
...
exports.convertArray = (carts) => {
  ...
  for (item of carts.cart) {
    ...
    cart.push({
      ...
      totalOfEachItem: totalOfEachItem,
      qty: item.qty,
    })
  }
  ...
  for (let otherItem of cartSessionTemp) {
    ...
    for (let item of carts.cart) {
      if (otherItem.productID === item.productID.toString()) {
        ...
        await cartModel.updateOne(
          { customerID: user.id, 'cart.productID': item.productID },
          { $set: { 'cart.$.qty': toItem } }
        )
        bool = true
        break
    ...
    if (!bool) {
      await cartModel.updateOne(
        ...
        {
          $push: {
            cart: { productID: otherItem.productID, qty: otherItem.qty },
          },
        }
      )
    }
